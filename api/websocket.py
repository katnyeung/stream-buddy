...
from core.script_manager import parse_script, analyze_progress, get_current_prompt, match_section_keywords, generate_response_streaming, _check_repetition, _update_buddy_history, strip_action_tags
...
def _use_reactive_brain() -> bool:
    """Check if reactive brain (fast filler while grok thinks) is enabled via env."""
...
def _periodic_brain_word_threshold() -> int:
    """Word count to auto-trigger brain during periodic transcription (0 = disabled)."""
    return int(os.getenv("PERIODIC_BRAIN_WORD_THRESHOLD", "0"))
...
async def process_brain_two_stage(
    ...
    """Two-stage brain: Stream Content → Parallel TTS (Stage 1 removed).

    Grok always runs as the real brain. Reactive filler fires in parallel at brain start.
    Returns a result dict compatible with analyze_progress() output.
    """
    # No Stage 1 — grok always generates content. Mood comes from action tags in response.
    stage1_result = {"mood": "friendly", "action": "respond"}

    # === Stream Content + Fire TTS per sentence ===
    sentences = []
    full_text = ""
    tts_tasks = []
    sentence_results = {}
    ...
    logging.info(f"[{session_id}] [TIMING] Two-stage complete: Stream+TTS={total_time:.2f}s, sentences={len(sentences)}")
...
# Timing configs
MIN_AUDIO_BYTES = 30000  # Minimum audio to transcribe (~3s of speech)
MIN_WORDS_FOR_BRAIN = 3  # Minimum words to filter noise (pause-based trigger)
...
async def websocket_stream(websocket: WebSocket, session: str = None):
    ...
    transcript_to_analyze = ""  # Current transcript queued for brain processing
    ...
    # Brain background processing (session-scope closure, callable by both VAD and periodic triggers)
    async def process_brain_background():
        nonlocal brain_processing, is_speaking, pending_response, last_prompt, last_remind_time, all_sections_covered, accumulated_transcript, transcript_to_analyze
        ...
        word_count = len(transcript_to_analyze.split()) if transcript_to_analyze else 0

        # Fire reactive filler immediately (parallel with grok reasoning)
        asyncio.create_task(try_reactive_response(
            transcript_to_analyze, "friendly", "nod"
        ))
        ...
        try:
            ...
            if _use_two_stage_brain():
                logger.info(f"[{session_id}] Two-stage brain started (background, {word_count} words, auto={auto_response_enabled})")
                ...
                if auto_response_enabled:
                    ...
                    # No WAIT branch — grok always responds, reactive fires at brain start

                else:
                    # Manual mode: stream content via grok, buffer TTS for hotkey
                    stage1_result = {"mood": "friendly", "action": "respond"}
                    ...
                    # Stream content, collect full text
                    full_text = ""
                    async for chunk in generate_response_streaming(
            ...
            logger.info(f"[{session_id}] Brain started (background, {word_count} words)")
            ...
            if action in ("respond", "enrich"):
                ...
                pass  # Reactive already fired at brain start
                        ...
                        if time_since_last >= periodic_transcribe_seconds:
                            ...
                            if filepath:
                                ...
                                if transcript and not is_hallucination:
                                    ...
                                    # === Periodic brain trigger (word count threshold) ===
                                    periodic_threshold = _periodic_brain_word_threshold()
                                    if periodic_threshold > 0:
                                        periodic_word_count = len(accumulated_transcript.split()) if accumulated_transcript else 0
                                        if (periodic_word_count >= periodic_threshold and
                                            not brain_processing and
                                            not is_speaking):
                                            logger.info(f"[{session_id}] Periodic brain trigger: {periodic_word_count} words >= {periodic_threshold}")

                                            # Keyword completion (same as VAD path)
                                            all_sections_covered, newly_completed, match_progress = await check_keyword_completion(
                                                session_id, full_transcript, all_sections_covered, threshold=0.5
                                            )
                                            state = await get_session_state(session_id) or {}
                                            ...
                                            last_brain_call_time = time.time()
